...
def check_if_relational_ref(name, elem_type):
    """Deterministically identifies if a field is an OCADS master pool pointer reference."""
...
def generate_library_code(root):
    ...
    for node in targets:
        ...
        code.append("    def __init__(self, instance_id='default', initial_values=None, className=None, style=None, **kwargs):")

        # 🎯 THE FIX: Place this assignment BEFORE layout_elements list declaration opens!
        ...
        for f in fields:
            ...
            if f['kind'] == 'primitive':
                ...
                if f['input_type'] == 'checkbox':
                    code.append(f"                    dbc.Checkbox(id={{'type': 'primitive-input', 'complex-type': '{type_name}', 'field': '{f['name']}', 'instance': instance_id}}, value=bool(init_vals.get('{f['name']}', False)))")
                else:
                    code.append(f"                    dbc.Input(id={{'type': 'primitive-input', 'complex-type': '{type_name}', 'field': '{f['name']}', 'instance': instance_id}}, type='{f['input_type']}', value=init_vals.get('{f['name']}', ''), placeholder='Enter {label}...')")
                code.append("                ], width=8),")
                code.append("            ], className='mb-2'),")

            elif f['kind'] == 'reference_selector':
                code.append("            html.Div([")
                code.append(f"                MetadataRefSelector(ref_field_name='{f['name']}', target_class='{f['target_class']}', parent_complex_type='{type_name}', instance_id=instance_id)")
                code.append("            ], className='mb-4'),")

            elif f['kind'] == 'list_group':
                code.append("            html.Div([")
                code.append(f"                MetadataListGroup(list_field_name='{f['field_link']}', component_class=globals().get('{f['target_class']}', html.Div), instance_id=instance_id)")
                code.append("            ], className='mb-4'),")
                ...
                code.append(f"                {child_class}(instance_id=instance_id, initial_values=initial_values, className='ms-3 border-start ps-3 bg-white') if globals().get('{child_class}') else html.Div(dbc.Input(id={{'type': 'primitive-input', 'complex-type': '{type_name}', 'field': '{f['name']}', 'instance': instance_id}}, type='text', placeholder='Enter {label}...'))")
...
if __name__ == '__main__':
    ...
    print("Success! Re-compiled reference-safe components.")